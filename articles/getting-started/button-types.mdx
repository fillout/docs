---
title: "Button types"
description: "Change the labels, alignment, design, and animation of your form's next, submit, and start buttons. Add a skip button and edit the position of your back button."
---

## List of button types

By default, your form will have the following buttons:

- **Next** **→** - assigned button for form, review, payment, login, and scheduling [pages](https://www.notion.so/Page-types-ac31ca2695c04b3b9881e7d62b62302c?pvs=21)
- **Submit** - button to proceed to the [ending page](https://www.notion.so/Ending-page-c472321e634e4ac5bd3b08104407619b?pvs=21)
- **Back** **←** - located on the upper left corner, allows respondents to view previous pages
- **Start** - found in [cover pages](https://www.notion.so/Cover-page-6f8a16bb5d284871b72274a12a837928?pvs=21)

You can also add:

- **Skip** - button that lets respondents proceed to the next page without answering the current one

## Edit button labels

Click a **button** followed by the `settings icon` to open the right-hand panel. Change the default **Button text** to your preferred label.

![5 Web](/images/5.webp)

## Change button alignment

In the right-hand panel settings, choose between the following **Alignment** options:

- **←** - left aligned
- **→|←** - center aligned
- **→** - right aligned
- **←|→** - expanded or same width as your form's text

## Customize button design and animation

The **Background color** of your button is determined by the [**Primary**](https://www.notion.so/Themes-f29afca7098d44eda5f076c16b490868?pvs=21) color you choose in your **Theme**. You can change its color through the right-hand panel.

<Check>
  **Good to know:** Changing the background color of your button will not change the primary color of your form.
</Check>

You can also change the shape of your button by clicking `Theme` on the upper left part of your form followed by `Advanced designer`.

![6 Web](/images/6.webp)

Under **Buttons**, choose a **Rounding** style to edit the shape. You can also choose `Grow` as your **Animation** style to make buttons increase a little in size when hovered over.

<Note>
  **Note:** Custom themes are available on the **Starter** plan and above. Learn more about our pricing plans and advanced features [**here**](https://www.fillout.com/pricing).
</Note>

## Change back button's position

In the **Advanced designer**, change the default **Back button position** to `Near next button`.

![7 Web](/images/7.webp)

## Add a skip button

Go back to your form's right-hand panel, toggle the `Skip button` switch.

![8 Web](/images/8.webp)

<Check>
  **Important:** Make sure your Back button position is set to Default (Top Left) to be able to use a Skip button.
</Check>

You can also let respondents automatically skip pages without the skip button by enabling [Auto jump](https://www.notion.so/Enable-auto-jump-on-your-form-449ebd871a2640ceb25d727dc77cd1be?pvs=21).

## **Related articles**

<CardGroup cols={3}>
  <Card title="Themes" icon="palette" iconType="regular" color="#FFC738">
    Design your form by selecting from hundreds of customizable themes or creating your own.
  </Card>
  <Card title="Page logic" icon="diagram-project" iconType="regular" color="#FFC738">
    Hide or show certain pages or custom endings based on respondents' previous input.
  </Card>
  <Card title="Auto-jump" icon="rabbit" iconType="regular" color="#FFC738">
    Automatically redirect respondents to the next page after making a selection.
  </Card>
</CardGroup>