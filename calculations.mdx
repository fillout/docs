...
Calculations in Fillout forms lets you compute values and respondent input, and display them. They are built using step-by-step rules rather than a single formula. Each rule performs one operation, and the result is passed to the next rule in sequence.
