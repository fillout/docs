...
Accessible form design covers a wide range of language, design, and technical choices. Although our team continues to incorporate accessibility best practices into our default settings, some accessibility improvements can only be implemented by you.
...
**Caption** and **Placeholder** text is always formatted in a smaller light-gray font that can be harder to read. For the best visibility, keep essential instructions, such as formatting requirements, in the field label. When a response needs to meet certain criteria, make sure to write a [custom error message](/input-validation#set-a-validation-rule) that begins with “Error:” and provides more details regarding validation requirements, including an example of an acceptable response.

  <img
    src="/images/guides/image-90.png"
    alt="Add a heading to the top of every form page to tell users what questions to expect"
    style={{ width:"87%" }}
  />

When asking Yes-or-No questions, restate what the user is saying yes or no to in the choices (e.g. “Yes, _I want to receive a quote from [Company]_”). And, in many cases, adding an “I’m not sure” option is helpful for users who struggle to interact with or understand everything included in the form.
