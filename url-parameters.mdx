...
<Steps>
  ...
  <Step title="Reference the parameter in your form">
    There are many [ways you can use URL parameters](/url-parameters#how-url-parameters-work). Type **@** and click the `link icon` <Icon icon="link" />, then choose your parameter. In this example, we'll show a respondent's name on the form.
...
<Tabs>
  <Tab title="Set it as a default value">
    ...
    In some cases, it can be useful to record which language a respondent used when answering your form. Follow the [steps above](/url-parameters#how-to-add-a-url-parameter-or-hidden-field) to create a URL parameter called "lang", then set `lang` as the field's **Default value**.
  </Tab>
